jQuery.massuploadcreate = function(){ 
    ...
    if (id) {
        ...
        html+= '</table>';
        html+= '</div>';
        $("form[name=total-form] #upload-area").append(html);
        $("form[name=total-form] #upload-area #upload-input-" + id).show('normal');
        $("#upid").attr({value:id});
    }
}
jQuery.massuploadremove = function(id){
    $("form[name=total-form] #upload-area #upload-input-" + id).remove();
}
