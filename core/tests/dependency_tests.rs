use danneo_core::registry::{PackageManifest, dependency::{resolve_dependencies, to_pubgrub_ver}};
use serde_json::json;
...
async fn test_dependency_resolver_success() {
    ...
    all_manifests.insert("settings".to_string(), vec![(SemanticVersion::new(2,0,0), settings_m)]);
    
    // news 1.0.0 depends on settings >=2.0.0
    let news_m = mock_manifest("mod_news", "1.0.0", vec![("settings", ">=2.0.0")]);
    all_manifests.insert("mod_news".to_string(), vec![(SemanticVersion::new(1,0,0), news_m)]);
    ...
    assert!(res.is_ok(), "Should resolve correctly when dependencies are met");
}
...
async fn test_dependency_resolver_missing_mandatory() {
    ...
    assert!(res.is_err(), "Should fail when mandatory dependency is missing");
}
...
async fn test_dependency_resolver_version_mismatch() {
    ...
    all_manifests.insert("settings".to_string(), vec![(SemanticVersion::new(1,0,0), settings_m)]);
    ...
    assert!(res.is_err(), "Should fail when dependency version is incompatible");
}
