...
use sea_orm::{ColumnTrait, ConnectionTrait, Database, EntityTrait, QueryFilter, Statement};
use serde_json::json;
use std::sync::Arc;
...
async fn test_mod_news_menu_registration_via_rpc() {
    ...
    let sections = tree["supercategories"]
        .as_array()
        .expect("supercategories should be array");
    ...
    if let Some(sec) = content_sec_opt {
        let news_cat_opt = sec["categories"]
            .as_array()
            .unwrap()
            .iter()
            .find(|c| c["code"] == "news");
        if let Some(news) = news_cat_opt {
            assert_eq!(
                news["items"].as_array().unwrap().len(),
                0,
