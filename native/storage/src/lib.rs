...
impl StorageModule {
    ...
    async fn get_bucket(&self, state: Arc<AppState>) -> Result<Bucket, String> {
        ...
        let region_obj = Region::Custom { region, endpoint };
    ...
    async fn call_rpc(
        ...
        match method {
            "upload" => {
                ...
                let data = if let Some(file_path) =
                    payload.get("file_path").and_then(|v| v.as_str())
                {
                    tokio::fs::read(file_path).await.map_err(|e| {
                        RpcError::Runtime(format!("Failed to read temp file: {}", e))
                    })?
                } else if let Some(content_base64) = payload.get("content").and_then(|v| v.as_str())
                {
                    ...
                    return Err(RpcError::BadRequest(
                        "Missing either 'file_path' or 'content'".to_string(),
                    ));
                };
...
register_native_module!("storage", |db| Arc::new(StorageModule::new(db)));
...
mod tests {
    ...
    #[danneotest]
    async fn test_storage_init(state: Arc<AppState>) {
        assert!(state.is_module_available("storage").await);
    }
}
