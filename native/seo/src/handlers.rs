...
use danneo_sdk::{
    apanel::render_admin_template, auth::Claims, models::core_settings, state::AppState,
};
...
pub async fn show_settings(
    ...
    render_admin_template(state, "apanel/seo_settings.html", context).await
}
...
pub async fn save_settings(
    ...
    prefixes.insert("faq".to_string(), sanitize_prefix(&form.prefix_faq, "faq"));
...
pub async fn show_sitemap(
    ...
    context.insert("changefreq_values", &changefreq_values());
    context.insert("priority_values", &priority_values());

    render_admin_template(state, "apanel/seo_sitemap.html", context).await
}
...
pub async fn save_sitemap(
    ...
    if let Err(e) = write_sitemap_file(&state, &sitemap).await {
        tracing::error!("Failed to write sitemap file: {}", e);
    }
...
pub async fn show_social(_claims: Claims, State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    render_admin_template(state, "apanel/seo_social.html", context).await
}
...
async fn write_sitemap_file(
    ...
    for item in item_iter(sitemap).filter(|item| item.add) {
...
fn item_iter(sitemap: &[SitemapModuleSetting]) -> impl Iterator<Item = &SitemapModuleSetting> {
    sitemap.iter()
}
...
fn sanitize_prefix(value: &str, fallback: &str) -> String {
    let slug = danneo_sdk::utils::seo::generate_slug(value);
    if slug.is_empty() {
        fallback.to_string()
    } else {
        slug
